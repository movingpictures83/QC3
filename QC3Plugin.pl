...
my $qc3ConfigFile = "plugins/QC3/qc3config.txt";
my %config;
...
#perl qc3.pl -m f -i inputfiles -o S12
...
$qc3ConfigFile = "plugins/QC3/qc3config.txt";
...
"Module (-m) is required and must be f (fastq), b (bam), and v (vcf)\n$usageModule"
	...
	die(
"Input file (-i) and Output directory (-o) must be provided\nYou can use '-h' to see help information for the module\n$usageModule"
	);
}
elsif ( !( -s $filelist ) ) {
	die(
...
"Something wrong in plotting figure. Please check the fastqSummary.rLog file!",
	...
	$template =
	  &build_template( $resultDir . "report_tmpl_fastq.tmpl",
		$reportHash );
